...
public class NpcState_QueueWait : IState
{
    ...
    public void Enter()
    {
        ...
        // 앞이든 아니든 계산대 쪽 보게 유지
        FaceToPoint(
            npcController.transform,
            queueManager.IsFront(npcController) ? queueManager.CounterTransform.position
                                                : queueManager.CounterNode.position
        );
    ...
    public void Tick()
    {
        if (npcController.QueueTarget == null) return;

        // 내 자리(myNode)가 바뀌면 이동
        if (npcController.QueueTarget != myNode)
        {
            npcController.Agent.ResetPath();
            npcController.Agent.isStopped      = false;
        ...
        int   bucket = Mathf.Clamp(Mathf.CeilToInt(speed), 0, 12);
        float dynTol = Mathf.Min(0.06f + 0.02f * bucket, 0.32f);
        ...
        if (agent.remainingDistance <= agent.stoppingDistance + dynTol)
        {
            if (!agent.hasPath || agent.velocity.sqrMagnitude <= 0.002f)
            {
                ...
                // ★앞자리면 즉시 결제 상태로 전환 (여기서 PlaceAllToCounter가 실행됨)
                if (queueManager.IsFront(npcController))
                {
                    npcController.stateMachine.SetState(
                        new NpcState_OfferPayment(npcController, queueManager.CounterTransform)
