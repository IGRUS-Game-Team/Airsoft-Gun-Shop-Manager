...
public class ExpansionTutorialManager : MonoBehaviour
{
    public static ExpansionTutorialManager Instance { get; private set; }
    ...
    [SerializeField] private string welcomeText = "Store Expansion Unlocked!";
    ...
    const string KeyDone = "ExpansionTutorialDone";
    ...
    void OnLevelChanged(int level)
    {
        ...
        if (extender != null && extender.MarketPurchased) return;
    ...
    {
        // ── Canvas ──
        _introCanvasRoot = new GameObject("ExpansionTutorialIntroOverlay");
    ...
    void SubscribeTutorialEvents()
    {
        ...
        TutorialEvents.OnMarketExpanded += HandleMarketExpanded;
        TutorialEvents.OnItemUnlocked += HandleItemUnlocked;
        TutorialEvents.OnPurchased += HandlePurchased;
        TutorialEvents.OnAllFurniturePlaced += HandleAllFurniturePlaced;
    }
    ...
    void HandleMonitorTabOpened(int index)
    {
        if (current == Step.OpenManagementTab && index == 4)
            TryAdvance();
        else if (current == Step.OpenUnlockTab && index == 5)
            TryAdvance();
        else if (current == Step.OpenShopTab && index == 2)
            TryAdvance();
    }

    void HandleMarketExpanded()
    {
        if (current == Step.PurchaseExpansion)
            TryAdvance();
    }

    void HandleItemUnlocked()
    {
        if (current == Step.UnlockShelf)
            TryAdvance();
    }

    void HandlePurchased()
    {
        if (current == Step.PurchaseShelf)
        {
            // 선반 구매 시 모니터가 자동으로 닫히므로 ExitMonitor 단계를 건너뜀
            current = Step.ExitMonitor;
            TryAdvance();
        }
    }
    ...
    void HandleAllFurniturePlaced()
    {
        if (current == Step.PlaceShelf)
            TryAdvance();
    }
    ...
    string GetGuideTextFor(Step step)
    {
        switch (step)
        {
            ...
            case Step.PurchaseExpansion:  return "Purchase the store expansion";
            case Step.OpenUnlockTab:     return "Open the Unlock tab";
            case Step.UnlockShelf:       return "Unlock a shelf";
            case Step.OpenShopTab:       return "Open the Stock Order tab";
            case Step.PurchaseShelf:     return "Purchase a shelf (Furniture)";
            case Step.ExitMonitor:       return "Exit the monitor (ESC)";
            case Step.PlaceShelf:        return "Place the shelf";
            case Step.Done:              return "Store expansion complete!";
        }
        return "";
    }
    ...
    //  디버그
    // ════════════════════════════════════════════
    [ContextMenu("DEBUG: Reset Expansion Tutorial")]
